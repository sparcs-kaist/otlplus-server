...
async function main() {
  const tsConfigPath = './apps/lab-server/tsconfig.app.json' // 실제 경로 설정
  ...
  const schemaSourceFiles = project.getSourceFiles('apps/lab-server/src/common/interfaces/**/*.ts')
  ...
  const sourceFiles = project.getSourceFiles('apps/lab-server/src/**/*.controller.ts')
  ...
  const outFileInSrc = path.join(process.cwd(), 'apps/lab-server/src', 'swagger.json')
  const outFileInDocs = path.join(process.cwd(), 'apps/lab-server/docs', 'swagger.json')
