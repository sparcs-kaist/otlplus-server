...
export class MeetingRepository {
  ...
  async createMember(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const timeblocks = await tx.meeting_member_timeblock.createMany({
  ...
  public async updateMemberSchedule(group_member_id: meeting_member['id'], schedule: TimeBlock[]) {
    return this.prisma.$transaction(async (tx) => {
      const falsedTimeblocks = await tx.meeting_member_timeblock.updateMany({
        where: {
