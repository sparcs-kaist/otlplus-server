import { TimeBlockDay } from '@otl/api-interface/src';
...
export const makeTimeIndexToTime = (timeIndex: number) => {
  return `${Math.floor(timeIndex / 2) + 8}:${timeIndex % 2 === 0 ? '00' : '30'}~${Math.floor(timeIndex / 2) + 8}:${timeIndex % 2 === 0 ? '30' : '00'}`;
};
