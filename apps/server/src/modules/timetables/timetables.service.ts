...
export class TimetablesService {
  ...
  async reorderTimetable(user: session_userprofile, timetableId: number, body: ITimetable.ReorderTimetableDto) {
    ...
    await Promise.all(
      timeTablesToBeUpdated.map(async (timetable) => {
        return this.timetableRepository.updateOrder(timetable.id, timetable.arrange_order);
      }),
    );
  ...
  public async getTimetableEntries(
    ...
    if (timetableId === MY_TIMETABLE_ID) {
      ...
      timetableDetails = await this.timetableRepository.getLecturesWithClassTimes(timetableId);
      if (!timetableDetails) {
