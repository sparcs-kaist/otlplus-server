...
export class Client {
  ...
  private validate_sign(payload: any[], timestamp: string, sign: string): boolean {
    ...
    if (!crypto.timingSafeEqual(Buffer.from(sign_client, 'utf-8'), Buffer.from(sign, 'utf-8'))) {
      return false
    }
    return true
  }
  ...
  public async get_user_info(code: string): Promise<ESSOUser.SSOUser> {
    ...
    */
    const [sign, timestamp]: [string, number] = this.sign_payload([code])
    const params = {
      client_id: this.client_id,
      code,
      timestamp,
    ...
    return await this.post_data(this.URLS.token_info, params)
  }
  ...
  public get_logout_url(sid: string, redirect_uri: string): string {
    ...
    */
    const [sign, timestamp]: [string, number] = this.sign_payload([sid, redirect_uri])
    const params = {
      client_id: this.client_id,
      sid,
      timestamp,
