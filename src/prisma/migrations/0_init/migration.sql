...
CREATE TABLE IF NOT EXISTS `subject_course` (
    ...
    `title_no_space` VARCHAR(100) GENERATED ALWAYS AS (REGEXP_REPLACE(`title`, '\\s+', '')) STORED,
    `title_en_no_space` VARCHAR(200) GENERATED ALWAYS AS (REGEXP_REPLACE(`title_en`, '\\s+', '')) STORED,
