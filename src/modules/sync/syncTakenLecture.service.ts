...
export class SyncTakenLectureService {
  ...
  async syncTakenLecture(data: ISync.TakenLectureBody) {
    ...
    const studentPairMap = new Map<
      number,
      [ETakenLecture.Basic[], number[], number[]]
    >();
    ...
    const userprofiles =
      await this.syncRepository.getUserProfileIdsFromStudentIds(studentIds);
    ...
    for (const [
      studentId,
      [existingTakenLectures, attendRecords, userprofileIds],
    ] of studentPairMap) {
      try {
        ...
        for (const userprofileId of userprofileIds) {
          ...
          for (const existing of existingTakenLectures.filter(
            (e) => e.userprofile_id === userprofileId,
          )) {
